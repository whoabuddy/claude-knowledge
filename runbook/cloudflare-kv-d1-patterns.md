...
**Three mechanisms leave an index missing while the schema still "looks" complete:**
...
3. **Never authored at all** — the plainest one. A table is created thinking of it as "keyed by X" (its PK), but a hot path also queries it by a *secondary* column that has no index. No drop, no failure: the index simply was never written. **Audit every `WHERE`/`ORDER BY` column, not just the PK.** Example: `arc0btc/agents-love-bitcoin` resolved every inbound email with `WHERE email_address = ?` on a directory table whose only index was the `btc_address` PK — a full scan of the entire directory per email (issue #21).
...
- **DO-embedded SQLite has no external `insights`**, and console doesn't reach worker-logs. Add a read-only **`schema-health` endpoint** that does two things: (a) diff live `sqlite_master` against an `EXPECTED_INDEXES` set, **and** (b) run `EXPLAIN QUERY PLAN` on each hot query, flagging any `SCAN` / `USE TEMP B-TREE FOR ORDER BY`. The expected-index diff only catches *dropped* indexes you knew to expect (mechanisms 1–2) — it cannot catch an index you never created (mechanism 3); the EXPLAIN pass is what catches that. Return missing names + offending plans + row counts. That's the DO equivalent of `insights`, and it doubles as the recurrence guardrail. (`aibtcdev/agent-news` `GET /api/config/schema-health`.)
...
**In a Durable-Object app, scan risk concentrates in the *singleton* DO — and a read-mostly singleton is usually a D1 table in disguise.** Per-key DOs (one per user / agent / room) are self-sharding: each instance holds one key's rows, so even an unindexed query scans a tiny table. The danger is the **singleton directory / index DO** that holds rows for *all* keys — that's the DO analog of "the one shared hot table," and it's *also* a global serialization point (one object, one CPU, every read funnels through it). Before reaching for a singleton DO, ask whether you actually need per-key serialization (a mailbox, a room, a globally-serialized counter — yes) or whether it's just read-mostly relational data (a directory, a resolution table, an index — no). Read-mostly directory data belongs in **D1**, where you get `insights`, `EXPLAIN`, indexes on any column, and `UNIQUE` constraints for free — all the things the silent-index failure modes above cost you inside a DO. (`arc0btc/agents-love-bitcoin` issue #21: the per-agent mailbox is correctly a per-key DO; the singleton directory DO should be D1.)
...
Source: May 2026 landing-page D1 attribution (PR `aibtcdev/landing-page#930`, −99.8% rows-read), the NewsDO follow-on (`aibtcdev/agent-news#827`), and the `arc0btc/agents-love-bitcoin` DO audit (issue #21: never-authored secondary-column index + singleton-DO-should-be-D1). The KV→D1 migration "traded a KV spike for a D1 spike" precisely because the inbox went from an O(1) KV key fetch to an O(N) unindexed scan — migrating storage backends does **not** fix a missing-index problem, it relocates it.
